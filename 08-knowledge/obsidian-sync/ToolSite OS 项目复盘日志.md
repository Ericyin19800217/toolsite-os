...
## 2026-06-10：子 agent 卡住问题排查复盘

### 关键进展

- 按系统化排查方式检查了子 agent 卡住问题。
- 当前上下文中残留的 `Bacon` 子 agent id 调用 `wait_agent` 后返回 `not_found`。
- 项目历史复盘中已有同类症状：子代理多次出现“文件已写入但不返回、不提交”。
- 新增排查报告：`08-knowledge/subagent-reliability-audit.md`。
- 更新全局调用地图：`subagent-driven-development` 从普通可选调整为“谨慎可选”。
...
```text
任务颗粒度过大
→ 子 agent 执行到文件写入 / 依赖安装 / 长任务后未稳定收口
→ 主线程等待或上下文恢复后只看到残留摘要
→ 最终表现为“子 agent 卡住”
```
...
之前把“有实施计划”理解成“适合交给子 agent 执行”。

实际上，计划可执行不等于适合子 agent。完整工具站 MVP 涉及脚手架、依赖安装、构建错误、测试、页面、QA 和提交，超出了子 agent 最稳定的短闭环范围。
...
- 主线程负责关键路径和最终收口。
- 子 agent 只做低耦合、短闭环、可独立验收的旁路任务。
- 子 agent prompt 必须包含硬收口格式：
...
- 如果子 agent 异常，不重复派同一任务，先由主线程检查 `git status`、文件内容、diff 和测试结果。

### 新原则

- 子 agent 不是替代项目负责人，而是临时协作者。
- 关键路径不能建立在不可恢复的子 agent 生命周期上。
- 所有 agent 工作都必须有明确退出条件。
- “完成”必须以文件、测试、提交和复验为准，不以 agent 自述为准。
